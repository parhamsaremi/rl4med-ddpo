...
from artifact_classifier import ArtifactClassifier  # Assuming this is the correct import path
...
def main(_):
    ...
    if not config.run_name:
        ...
        if config.resume_from is None or config.resume_from == "":
            assert not os.path.exists(os.path.join(config.logdir, config.run_name))
    ...
    model = ArtifactClassifier(num_classes=6).to(unet.device)
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        if epoch % config.save_freq == 0 and accelerator.is_main_process:
            accelerator.save_state(output_dir=os.path.join(config.logdir, config.run_name, f"checkpoint_{epoch}"))
