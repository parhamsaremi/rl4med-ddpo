...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      let count = await env.RL4MED_COUNTER.get('total_visitors');
      if (count === null) {
      ...
      if (path === '/increment') {
        ...
        const hasVisited = await env.RL4MED_COUNTER.get(ipKey);
        
        if (!hasVisited) {
          // This is a new IP, increment the counter
          count += 1;
          await env.RL4MED_COUNTER.put('total_visitors', count.toString());
          
          // Mark this IP as counted (with 30-day expiration)
          await env.RL4MED_COUNTER.put(ipKey, 'visited', {expirationTtl: 60 * 60 * 24 * 30});
        }
      }
